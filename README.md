...
A WebGL tie-dye game for a baby reveal. Guests choose a spiral, accordion, or scrunch fold, tie three rubber bands, and paint with three grayscale dye shades. Opening the shirt reveals a pink or blue pattern shaped by their dye placement. They can add a name, title, and sticker, download a PNG, and hang it on a shared clothesline.
...
Copy `.env.example` to `.env.local` and set `VITE_REVEAL=pink` or `VITE_REVEAL=blue`. Restart the development server after changes.

With no valid value, the game shows a **clearly labeled blue demo**. For deployment, set the repository variable `VITE_REVEAL` under **Settings → Secrets and variables → Actions → Variables**, then run the deployment workflow again. Demo shirts will not appear in the real reveal's gallery.
...
4. Set `VITE_REVEAL` when ready for the actual reveal; leave it empty for the demo.
